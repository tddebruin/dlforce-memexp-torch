function [ latex2 ] = noise_table( experimentcell, truemean, truestdv, index_start, index_end, caption, label )
    ...
    input.data = cell(length(experimentcell), 6);
    input.tableColLabels = {'$\mu$','$\sigma$','$\mu$','$\sigma$','$\mu$','$\sigma$'};
    ...
    for dbstrat = 1:length(experimentcell)
        ...
        input.data{1,1} = num2str(truemean , format );
        input.data{1,2} = num2str(truestdv , format );
        input.data{1,3} = num2str(truemean , format );
        input.data{1,4} = num2str(truestdv , format );
        input.data{1,5} = num2str(truemean ,format );
        input.data{1,6} = num2str(truestdv , format );
        
        
        input.tableRowLabels{dbstrat+1} = dbsname;
        input.data{dbstrat+1,1} = num2str(mean(s1) , format );
        input.data{dbstrat+1,2} = num2str(std(s1) , format );
        input.data{dbstrat+1,3} = num2str(mean(s2) , format );
        input.data{dbstrat+1,4} = num2str(std(s2) , format );
        input.data{dbstrat+1,5} = num2str(mean(a) , format );
        input.data{dbstrat+1,6} = num2str(std(a) , format );
...
% % % Now call the function to generate LaTex code:
 ...
 latex2{5} = '& \multicolumn{2}{c}{Position} & \multicolumn{2}{c}{Velocity} & \multicolumn{2}{c}{Action} \\';
 latex2{6} = latex{5};%[column1str ' & mean & \Good & mean, stdv of \Good & mean & \Good & mean, stdv of \Good \\'];
 for i=7:7+length(experimentcell) + 6
     latex2{i} = latex{i-1};
 end
 clc;
