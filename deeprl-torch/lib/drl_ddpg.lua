...
function ddpg:train(experiencedb, trainsettings)
  parent:train(experiencedb, trainsettings)
	...
	local dbrequests, requested_batches
	--[[if trainsettings.prioritized_experience_replay then
		dbrequests 					= trainsettings.batchupdates
		requested_batches 	= 1 
	else
		]]
		dbrequests 					= 1
		requested_batches 	= trainsettings.batchupdates
	--end
	self.updateCount      = self.updateCount + 1

	for dbreq = 1,dbrequests do
		self.trainbatch				= self.experiencedb:get_mini_batch({
			batch_size 			= trainsettings.batch_size,
			maxbatches			= requested_batches,
			GPU 						= GPU,
	  ...
	  if trainsettings.logUseCount then    
			experiencedb:update_extra_info("USECOUNT",self.trainbatch.db_indices:clone():double():fill(1),self.trainbatch.db_indices,"increment")
		end
...
function ddpg:train_noseq(batch_index,state_index,gamma,train_settings, experiencedb)
	local sequence_index = 1
		...
		function evaluateQ(paramx_)
			...
			local next_a 
			if opt.doubleQ then
				next_a  	= self.train_networks.actor.network:forward(next_s) 	
			else
				next_a  	= self.train_networks.FROZENactor.network:forward(next_s) 	
			end
			local is_weights
			if self.trainbatch.is_weights then
				is_weights = self.trainbatch.is_weights[{batch_index,sequence_index}]
			else
				is_weights = nil
			end
...
function ddpg:createCheckpoint()
	self.checkpoint 			= {}
...
function ddpg:resetToLastCheckpoint()
