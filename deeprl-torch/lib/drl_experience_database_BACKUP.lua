...
function em:__init(short_term_memory,long_term_memory,settings)
	...
	if long_term_memory then 
		...
		self.experience_database = {current_write_index = 1, last_write_index = 0}
...
function em:get_mini_batch(batch_settings)
	...
	if seq_properties.length > 1 then
		...
		if prioritized and availableIndices:nElement() > batch_size*batches then -- rank based prioritized experience replay ( https://arxiv.org/abs/1511.05952 )			
			...
			local bucket_boundaries = {}
			is_weights = {}
			bucket_boundaries[0] = 0
			local chosen_idcs = {}
			for bb = 1,batch_size do
				bucket_boundaries[bb] = math.max(bucket_boundaries[bb-1]+1,
					availableIndices[sampling_probabilities:ge(sampling_probabilities[-1]*bb/batch_size)]:min())
				chosen_idcs[bb] = math.random(bucket_boundaries[bb-1]+1,bucket_boundaries[bb])
				is_weights[bb] = batch_size/availableIndices:nElement() * (bucket_boundaries[bb] - bucket_boundaries[bb-1])	
			end

			

			local tdevals = self:get_extra_info('TDE')
			tdevals = tdevals:index(1,availableIndices:long())
			local sorted_tdevals
			local sorted_indices
			sorted_tdevals, sorted_indices = torch.sort(tdevals,true)
			batchIndices = availableIndices:index(1,sorted_indices:index(1,torch.LongTensor(chosen_idcs)):long()):long()
			if prioritized_beta > 0 then
				batch.is_weights = torch.Tensor(is_weights)
				batch.is_weights:pow(prioritized_beta)
				batch.is_weights:div(batch.is_weights:max())
			end
		else
