...
local em = torch.class('experience_memory_datadump')
		...
		if prioritized and availableIndices:nElement() > batch_size*batches then -- rank based prioritized experience replay ( https://arxiv.org/abs/1511.05952 )			
			...
			for bb = 1,batch_size*batches do
				...
				if countbasedis then
					...
					self.per.counts = self:get_extra_info('COUNT')
				else
			...
			for i = 1,batchIndices:nElement() do
				...
				if countbasedis then
					self.per.is_weights[bb] = 1/(self.per.counts[batchIndices[i]]+1)						
				end
...
function em:lognoise(batch)
 	self.lastsampledstate1noise = self:get_extra_info('STATENOISE_1'):index(1,batch.db_indices:clone():resize(batch.db_indices:nElement()):long())
 	self.lastsampledstate2noise = self:get_extra_info('STATENOISE_2'):index(1,batch.db_indices:clone():resize(batch.db_indices:nElement()):long())
 	self.lastsampledactionnoise = self:get_extra_info('ACTIONNOISE'):index(1,batch.db_indices:clone():resize(batch.db_indices:nElement()):long())
 	print('log')
end

function em:save_noise_stats(filename)
 	noisedata = {state1 = self.lastsampledstate1noise, state2 = self.lastsampledstate2noise, action = self.lastsampledactionnoise }
 	mattorch.save(filename, noisedata)
 	print('save ' .. filename)
end
